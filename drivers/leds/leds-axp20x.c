...
static int axp20x_led_parse_dt(struct axp20x_led *priv, struct device_node *np)
{
	...
	mode = of_get_property(np, "x-powers,charger-mode", NULL);
	if (mode) {
		priv->ctrl = AXP20X_CHGLED_CTRL_CHARGER;
		if (!strcmp(mode, "mode-a"))
			priv->mode = AXP20X_CHGLED_MODE_A;
		else if (!strcmp(mode, "mode-b"))
			priv->mode = AXP20X_CHGLED_MODE_B;
