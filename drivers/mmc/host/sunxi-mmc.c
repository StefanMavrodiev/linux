...
#define SDXC_REG_CSDC		0x054 /* SMHC CRC Status Detect Control */
#define SDXC_REG_A12A		0x058 /* SMC Auto Command 12 Register */
#define SDXC_REG_SD_NTSR	0x05C /* SMC New Timing Set Register */
#define SDXC_REG_DDR_SBIT_DET	0x10C /* SMC Start Bit Detection control */
...
#define SDXC_HS_MD_EN		BIT(31)
#define SDXC_HALF_START		BIT(0)

#define SDXC_CRC_DET_HS400	0x06
#define SDXC_CRC_DET_OTHER	0x03
...
{
	unsigned long expire = jiffies + msecs_to_jiffies(100);
	u32 rval;
	u8 delay;
	 ...
	 * Control Unit(CCU). Calibration delay chain a internal function in SMHC and don’t need device. So, it is
	 ...
	 * of SMHC’s clock nearly. This value is the result of calibration.
	 *
	 * Step6: Calculate the delay time of one delay cell according to the cycle of SMHC’s clock and the result of
	 * calibration.
	*/

	dev_info(mmc_dev(host->mmc), "Precondition calibration.\n");
	writel(SDXC_CAL_DL_SW_EN | 0x20, host->reg_base + reg_off);
	writel(0, host->reg_base + reg_off);

	dev_info(mmc_dev(host->mmc), "Starting calibration.\n");
	writel(0x8000, host->reg_base + reg_off);

	do {
		rval = readl(host->reg_base + reg_off);
	} while (time_before(jiffies, expire) && !(rval & SDXC_CAL_DONE));

	if (!(rval & SDXC_CAL_DONE)) {
		dev_info(mmc_dev(host->mmc), "Calibration not ready!\n");
		return 0;
	}

	delay = (rval >> 8) & 0x3F;

	dev_info(mmc_dev(host->mmc), "Calibration done!\n");
	dev_info(mmc_dev(host->mmc), "SAMP_DL: 0x%02X\n", delay);

	dev_info(mmc_dev(host->mmc), "Setting delay to %d\n", delay/4);
	writel(SDXC_CAL_DL_SW_EN | (delay / 4), host->reg_base + reg_off);
...
static void sunxi_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_TIMING_MMC_DDR52:
		dev_dbg(mmc_dev(host->mmc), "Setting mode to DDR\n");
		mmc_writel(host, REG_DDR_SBIT_DET, 0);
		mmc_writel(host, REG_GCTRL, mmc_readl(host, REG_GCTRL) | SDXC_DDR_MODE);
		mmc_writel(host, REG_CSDC, SDXC_CRC_DET_OTHER);
		break;

	case MMC_TIMING_MMC_HS400:
		dev_dbg(mmc_dev(host->mmc), "Setting mode to HS400\n");
		/*
		...
		/*
		 * When HS_MD_EN is set, CCLK_DIV field must be cleared.
		 */
		mmc_writel(host, REG_CLKCR, mmc_readl(host, REG_CLKCR) & (~0x000F));

		/* Enable HS400 */
		mmc_writel(host, REG_DDR_SBIT_DET, SDXC_HS_MD_EN | SDXC_HALF_START);
		mmc_writel(host, REG_CSDC, SDXC_CRC_DET_HS400);
		break;

	default:
		dev_dbg(mmc_dev(host->mmc), "Setting mode to %d\n", ios->timing);
		mmc_writel(host, REG_DDR_SBIT_DET, 0);
		mmc_writel(host, REG_GCTRL, mmc_readl(host, REG_GCTRL) & (~SDXC_DDR_MODE));
		mmc_writel(host, REG_CSDC, SDXC_CRC_DET_OTHER);
		break;
...
static int sunxi_mmc_prepare_hs400_tuning(struct mmc_host *mmc, struct mmc_ios *ios)
{
	struct sunxi_mmc_host *host = mmc_priv(mmc);
	dev_info(mmc_dev(mmc), "sunxi_mmc_prepare_hs400_tuning\n");

	dev_info(mmc_dev(mmc), "%s(): clock: %d\n", __func__, ios->clock);
	dev_info(mmc_dev(mmc), "%s(): vdd: %d\n", __func__, ios->vdd);
	dev_info(mmc_dev(mmc), "%s(): power_mode: %d\n", __func__, ios->power_mode);
	dev_info(mmc_dev(mmc), "%s(): bus_width: %d\n", __func__, ios->bus_width);
	dev_info(mmc_dev(mmc), "%s(): timing: %d\n", __func__, ios->timing);

	return 0;
}

static int sunxi_mmc_get_delay(struct mmc_host *mmc, int reg_off)
{
	unsigned long expire = jiffies + msecs_to_jiffies(100);
	struct sunxi_mmc_host *host = mmc_priv(mmc);
	u32 rval;
	...
	if (!(rval & SDXC_CAL_DONE)) {
		dev_err(mmc_dev(mmc), "Calibration delay timeout!\n");
		return -ETIME;
	}

	delay = (rval >> 8) & 0x3F;
	dev_info(mmc_dev(mmc), "Calibration value: %d\n", delay);
...
static int sunxi_mmc_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	/* We only care for HS200 and HS400 */
	if (mmc->actual_clock <= 52000000)
		return 0;


	dev_dbg(mmc_dev(mmc), "Actual clock: %d\n", mmc->actual_clock);
	dev_dbg(mmc_dev(mmc), "Starting sample delay tuning procedure...\n");

	delay = sunxi_mmc_get_delay(mmc, SDXC_REG_SAMP_DL_REG);
	if (delay < 0)
		return delay;

	// for(i = 0; i <= delay; i++) {
	// 	mmc_writel(host, REG_SAMP_DL_REG, SDXC_CAL_DL_SW_EN | i);
	// 	mmc_writel(host, REG_DS_DL_REG, SDXC_CAL_DL_SW_EN | i);
        //
	// 	ret = mmc_send_tuning(mmc, opcode, NULL);
	// 	dev_info(mmc_dev(mmc), "Trying delay: %d ... %d", i, ret);
        //
	// 	if (!ret)
	// 		break;
	// }

	mmc_writel(host, REG_SAMP_DL_REG, SDXC_CAL_DL_SW_EN | host->sample_delay);
	mmc_writel(host, REG_DS_DL_REG, SDXC_CAL_DL_SW_EN | host->strobe_delay);
...
static const struct mmc_host_ops sunxi_mmc_ops = {
	...
	.prepare_hs400_tuning = sunxi_mmc_prepare_hs400_tuning,
...
static int sunxi_mmc_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(pdev->dev.of_node, "sunxi,sample_delay", &host->sample_delay);
	of_property_read_u32(pdev->dev.of_node, "sunxi,strobe_delay", &host->strobe_delay);

	dev_info(&pdev->dev, "sample: %d strobe:%d\n",host->sample_delay, host->strobe_delay);
